...
#define TIME 1000                                // culture duration in the second phase
...
void stationary_phase(double tau1, double tau2, int trial){
    ...
    char fname1[50];
    sprintf(fname1,"./data/non_steady_%.3f/time.csv", E_mean_alp);
    std::ofstream ofs3(fname1); // time points
    char fname2[50];
    sprintf(fname2,"./data/non_steady_%.3f/volume_%.0f_%.0f_%d.csv", E_mean_alp, tau1, tau2, trial);
    std::ofstream ofs4(fname2); // the list of the cell volume
...
int main(int argc, char *argv[]){
    ...
    for(int trial=0; trial<5; trial++){
        ...
        std::cout << trial+1 << "/5 steps completed." << std::endl;
    }
