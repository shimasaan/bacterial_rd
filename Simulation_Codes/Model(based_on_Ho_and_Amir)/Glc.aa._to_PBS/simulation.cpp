...
#define TIME_E 150                             // culture duration in the initial phase
#define TIME 200                                // culture duration in the second phase
...
#define shuffle_cellamount 5000                   // number of cells at the onset of starvation
#define max_cellamount 200000                   // cells are not added when the population exceeds this value


//===========DISTRIBUTION PARAMETERS============//
#define initiation_vol 0.45                      // 0.9, if completely consistent with the M9 experiment (Wallden et al. 2016) (FREE)
#define sigma_Ithre_coef 0.1                     // std/mean of C period initiation volume (FREE) 0.2

#define E_mean_alp 0.01                            // mean growth rate
...
#define E_mean_rep_speed 1.0/(1.3*std::pow(E_mean_alp, -0.84)+42)
#define E_mean_vol 1.8
...
void pre_exponential_phase(){
    while(cell_count < initial_cellmax){
        ...
        for(int i=0; i<cell_count_s; i++){
            t2[i] = t2[i]+DeltaT*E_mean_rep_speed;
            t4[i] = t4[i]+DeltaT*E_mean_rep_speed;
            t8[i] = t8[i]+DeltaT*E_mean_rep_speed;
            t16[i] = t16[i]+DeltaT*E_mean_rep_speed;
...
void exponential_phase(){
    ...
    while(t < 1000){ // shuffle the cell cycle across individuals
        t = t + DeltaT; //update the external clock
...
void stationary_phase(double kc, double tau1){
    ...
    while(t < TIME){
        ...
        for(int i=0; i<cell_count; i++){
            alp[i] = meanalp;
            rep_speed = E_mean_rep_speed*(kc+1)/(kc*std::exp(t/tau1)+1);
